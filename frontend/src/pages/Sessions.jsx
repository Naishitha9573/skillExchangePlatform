...
export default function Sessions(){
  ...
  return <main className="container workspace-page sessions-page page-transition">
    ...
    <section className="schedule-list"><div className="schedule-list-heading"><h2>Your learning sessions</h2><button className="ghost icon-button" aria-label="Refresh sessions" onClick={load}><RefreshCw size={16}/></button></div><div className="session-tabs" aria-label="Filter sessions">{[['upcoming','Upcoming',upcoming.length],['past','Past',past.length],['cancelled','Cancelled',cancelled.length]].map(([key,label,count])=><button key={key} aria-pressed={tab===key} className={tab===key?'active':''} onClick={()=>setTab(key)}>{label}<span>{count}</span></button>)}</div>
      ...
      {!loading&&visible.length<3&&<section className="session-preparation"><div className="section-kicker">A great session starts before you join</div><h3>Bring a question. Leave with a new skill.</h3><div>{[['01','Pick one small goal','Agree on something you can explore in one session.'],['02','Bring something to try','A question, a work in progress, or a practical example.'],['03','Leave room to reflect','Share what clicked and choose your next step together.']].map(([number,title,text])=><article key={number}><span>{number}</span><div><strong>{title}</strong><p>{text}</p></div></article>)}</div><Link className="text-link" to={next?`/messages/${next.organizer.id===user.id?next.participant.id:next.organizer.id}`:'/messages'}>Share your plan with a partner <ArrowRight size={14}/></Link></section>}
    </section></div>
